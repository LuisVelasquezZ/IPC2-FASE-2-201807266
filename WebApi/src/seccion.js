...
function createRouter(db) {
  ...
  router.get('/seccionCurso/:idCurso', function (req, res, next) {
    ...
    db.query(
        'SELECT idSeccion, Seccion.nombre, horaInicio, horaFin, Usuario.nombre as auxiliar, Usuario.idUsuario, semestre, anio, estado, observacion'
  ...
  router.get('/seccion/:idSeccion', function (req, res, next) {
    db.query(
        'SELECT idSeccion, Seccion.nombre, horaInicio, horaFin, Usuario.nombre as auxiliar, Usuario.idUsuario, semestre, anio, estado, observacion FROM seccion  INNER JOIN Usuario ON Seccion.idUsuario = Usuario.idUsuario WHERE idSeccion = ?',
  ...
  router.put('/seccion/:idSeccion', function (req, res, next) {
    db.query(
      'UPDATE seccion SET nombre=?, idCurso=?, horaInicio=?, horaFin=?, idUsuario=?, semestre=?, anio=?, estado=?,  observacion=?  WHERE idSeccion=?',
      [req.body.nombre, req.body.idCurso, req.body.horaInicio, req.body.horaFin, req.body.idUsuario, req.body.semestre, req.body.anio, req.body.estado, req.body.observacion,req.params.idSeccion],
      (error) => {
        if (error) {
          res.status(500).json({status: 'error'});
        } else {
