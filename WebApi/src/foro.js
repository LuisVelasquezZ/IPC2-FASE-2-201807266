...
function createRouter(db) {
  ...
  router.post('/publicacion', (req, res, next) => {
    db.query(
      ...
      [req.body.fechaLimite, req.body.contenido, req.body.idUsuario, req.body.idSeccion],
      (error) => {
        if (error) {
          console.error(error);
  ...
  router.get('/publicacionSeccion/:idSeccion', function (req, res, next) {
    db.query(
      'SELECT idPublicacion, fechaLimite, fecha, titulo, contenido, idUsuario, idSeccion FROM publicacion WHERE idSeccion=?',
